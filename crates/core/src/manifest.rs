...
//! **Note-funded:** `dexdo provision` brings up the OB + RootModel + per-deal `TokenContract`
...
//! one-time mint faucet only; zero giver in the operate path.
...
/// non-empty `--`-separated parts(e.g. `provider--model--v1`, `qwen--qwen3--32b`). This is the string
...
pub fn validate_canonical_model_id(name: &str) -> Result<(), String> {
    ...
    Err(format!(
        "model id `{name}` is not canonical `producer--model--version` (exactly three non-empty `--`-separated \
         parts, e.g. `provider--model--v1`). This string is the on-chain model name/hash the indexer \
...
mod tests {
    ...
    fn validate_canonical_model_id_requires_three_parts() {
        assert!(validate_canonical_model_id("provider--model--v1").is_ok());

...
/// Active sell orders are keyed by `TokenContract`: two resting asks for the same TC would present one escrow
/// endpoint as independent liquidity twice. Return a deterministic diagnostic for any duplicate TC.
pub fn duplicate_resting_ask_token_contracts(orders: &[OrderBookOrder]) -> Vec<TokenContract> {
    let mut seen = BTreeSet::new();
    let mut duplicates = BTreeSet::new();
    for order in orders.iter().filter(|o| o.is_resting_ask()) {
        let Some(tc) = order.token_contract.as_deref() else {
            continue;
        };
        let key = tc.to_ascii_lowercase();
...
pub fn check_no_duplicate_resting_asks(orders: &[OrderBookOrder]) -> Result<(), String> {
    let duplicates = duplicate_resting_ask_token_contracts(orders);
    if duplicates.is_empty() {
        return Ok(());
    }
    let mut ids_by_tc: BTreeMap<String, Vec<u128>> = BTreeMap::new();
    for order in orders.iter().filter(|o| o.is_resting_ask()) {
        if let Some(tc) = order.token_contract.as_deref() {
            ids_by_tc
                .entry(tc.to_ascii_lowercase())
    ...
    let details = duplicates
        ...
        .map(|tc| {
            ...
            format!("{tc} at order_ids [{ids}]")
        })
        .collect::<Vec<_>>()
        .join("; ");
    Err(format!(
        "duplicate active sell orders for one TokenContract are forbidden: {details}. \
         Cancel/fill/cleanup the old order before reposting; duplicates must not be quoted as independent \

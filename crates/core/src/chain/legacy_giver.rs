...
/// The manifest the live operator tools in this file run against.

/// Each of these used to carry a path compiled in, naming one chain's file next to the source. That
/// made the chain a property of the BUILD: the tool ran against whatever the binary was compiled
/// with, an operator holding a second manifest had no way to point it there short of editing and
/// rebuilding, and the run printed a chain name it had not read from anywhere. `DEXDO_MANIFEST` is
/// the single thing that says which network the client talks to everywhere else, so it says it here
/// too -- and if it is unset these refuse, rather than reaching for a chain nobody named.
fn live_manifest() -> PathBuf {
    crate::params::manifest_path().unwrap_or_else(|refusal| panic!("{refusal}"))
}

/// What one note costs the wallet that deploys it: the smallest nominal (N100) plus the collection
/// `RootPN` takes at birth. Derived, not written out -- as `450_000_000_000` it described the deploy
/// when it had a second leg (a gas voucher of 100 SHELL on top), and after that leg was removed the
/// figure over-budgeted every wallet by 100 SHELL and recorded that number as evidence.
const WALLET_REFILL_NOTE_COST_RAW: u128 =
    100 * crate::params::SHELL_UNIT + crate::params::ROOT_PN_GAS_DEPOSIT_RAW as u128;
...
fn ensure_giver_mint_faucet_ready(
    ...
    let Some(observation) = observation else {
        return Err(anyhow!(
            "configured the Giver is missing; planned mint {planned_mint_raw} raw ECC[2] was not submitted"
        ));
    };
    if observation.dapp_id != GIVER_SYSTEM_DAPP_ID {
        return Err(anyhow!(
            "configured the Giver is in dapp {}, not the all-zero system dapp; planned mint {planned_mint_raw} raw ECC[2] was not submitted",
            observation.dapp_id
        ));
    }
    if !observation.active {
        return Err(anyhow!(
            "configured the Giver is not Active; planned mint {planned_mint_raw} raw ECC[2] was not submitted"
        ));
    }
    if observation.code_hash.as_deref().is_none_or(str::is_empty) {
        return Err(anyhow!(
            "configured the Giver has no live code hash; planned mint {planned_mint_raw} raw ECC[2] was not submitted"
        ));
    }
    Ok(())
}
...
/// cargo test -p dexdo-core live_post_seller_bond -- --ignored --nocapture`
#[tokio::test]
...
async fn live_post_seller_bond_note_funded() {
    ...
    let be = RealChainBackend::connect(manifest).expect("connect to the chain");
...
/// A LIVE read-only test against the real Acki Nacki test chain: connection,
...
/// Run: `cargo test -p dexdo-core -- --ignored --nocapture`.
#[tokio::test]
#[ignore = "live: hits the real Acki Nacki Block Manager (read-only)"]
async fn live_chain_connect_and_read() {
    // The manifest lives at the workspace root; cargo runs the test from the crate's directory.
    let manifest = live_manifest();
    let be = RealChainBackend::connect(manifest).expect("connect to the chain + load manifest");
    ...
    // The SuperRoot from the manifest must be an active account on the chain.
    let acc = be
        .client()
    ...
    assert!(
        ...
        "SuperRoot must be active on the chain"
    );
...
/// the deterministic multisig deploy address and **mints test SHELL from the test giver**
/// (`0:1111...`, keys in the SDK config). Removes the former blocker "a funded wallet from the
/// coordinator is required".
/// This is the first REAL write against the chain -- it checks that submit goes through from this environment.
#[tokio::test]
...
async fn live_giver_funds_fresh_wallet() {
    ...
    let endpoint_owned = crate::params::current_network_endpoint().expect("manifest endpoint");
...
/// on the chain. The path to the pool (owner keys; the secrets are gitignored, outside the repo) is taken from
/// `DEXDO_PN_POOL`; without it the test is skipped. Run:
/// `DEXDO_PN_POOL=/abs/pn_pool.json cargo test -p dexdo-core -- --ignored --nocapture live_minted_note`.
#[tokio::test]
#[ignore = "live: deploys an InferenceOrderBook on the chain from a minted note (a real write)"]
async fn live_minted_note_deploys_inference_orderbook() {
    ...
    assert!(
        active,
        "InferenceOrderBook is deployed and active on the chain"
    );
    ...
    async fn wait_voucher_extout(
        ...
        loop {
            let resp: serde_json::Value = be
                .http
                .post("https://net-a.example/graphql")
                .json(&json!({
                    "query": query,
                    "variables": {
...
async fn diag_412_repin_fresh_note_passes() {
    ...
    let res = note_code_hash_current(
        PRIVATENOTE_PINNED_CODE_HASH,
        &note,
        acc.code_hash.as_deref(),
    );
...
#[tokio::test]
#[ignore = "live: deploys a TokenContract + posts a sell offer on the chain (real writes)"]
...
#[tokio::test]
#[ignore = "live:  wallet-funded market provisioning on the chain (writes; deploys OB/RM/TC)"]
async fn live_provision_wallet_funded_market() {
    ...
    let manifest_path = live_manifest();
...
#[tokio::test]
#[ignore = "live: buyer matches a resting offer on the chain (writes; match funds seller TC)"]
async fn live_buyer_matches_offer() {
    ...
    let escrow = crate::market::required_escrow_for_buy(ticks, price);
    let resp = be
        .place_inference_buy(
            &buyer_addr,
            &buyer,
...
#[tokio::test]
#[ignore = "live: stream open + handover + buyer stop-on-probe (ProbeBurned) on the chain"]
async fn live_stream_open_and_probe_burn() {
    ...
    let (_ob, tc) = setup_funded_deal(
        ...
        5,
        2,
        crate::market::required_escrow_for_buy(2, price),
    )
    .await;
...
/// on the chain -- checks the wiring of step 5 end-to-end. Slow (~6 min: 185s SETTLE_WINDOW). Requires `DEXDO_PN_POOL`.
#[tokio::test]
...
async fn live_real_deal_backend_trait() {
    ...
    let ctx = DealContext {
        ...
        max_ticks,
        ticks,
        escrow: crate::market::required_escrow_for_buy(ticks, price),
    };
...
#[tokio::test]
#[ignore = "live:  recover STOPs an orphaned OPEN streaming deal (AmicableSplit) on the chain (~6min)"]

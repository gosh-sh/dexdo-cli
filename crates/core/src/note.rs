//! Note trait and crypto handover (§3.1, §4.5).
//!
...
/// Note — a shielded wallet with handover cryptography (§4.5).
///
...
pub trait Note: Send + Sync {
    ...
    /// Encrypt `msg` to the `peer` pubkey (endpoint to the buyer's pubkey, §3.1).
    ...
    /// Sign a message with the note's private key (stream-session challenge, §3.1.1).
    fn sign(&self, msg: &[u8]) -> Signature;
}

/// Verify a signature against the note pubkey (ed25519). Gateway side (§3.1.1).
...
/// Derive the x25519 handover pubkey from the note's ed25519 pubkey (§3.1 + D10): one note key sets
...
/// its `seed` in memory — this is the **tree root** (HD-style, directive 7): child (sub)notes are
/// derived by index via [`LocalNote::derive`]. The ephemeral `generate()` and the child notes
/// themselves carry no seed (flat tree of depth 1). The seed is never written to disk (custody is external, §5).
...
impl LocalNote {
    /// Generate a new note from the system RNG. Directive 7: this is an **ephemeral** note —
    ...
    ///
    /// **NOTE (D10 F6): x25519 here is an HKDF derivative, ≠ [`LocalNote::from_ed25519_signing`]**
    ...
    /// `generate()` and for child notes. dexdo holds the seed only in memory and never writes it to disk (§5).
    ...
    /// is an external module/wallet (§5). A wrong key → an explicit error, not silent generation.
    ...
    /// pubkey from on-chain ed25519 — no separate x25519 channel is needed. Used by `RealNote` (§3.1, one
    /// note key on the chain). The x25519 secret = the ed25519 signing scalar (x25519 clamps idempotently).
    ///
    /// **NOTE (D10 F6): ≠ [`LocalNote::from_seed`].** `from_seed` derives x25519 via **HKDF**
...
/// (`node`/`nodes`): nothing is written to disk (custody is an external module/wallet, §5). Enumerating
...
impl NoteTree {
    ...
    /// (directive 7, R14: state across ALL notes under the key). "From whom" = the note pubkey (§2.2).
...
/// Domain string of the (sub)note-by-index derivation KDF (directive 7, HD tree) — separates the child
...
impl Note for LocalNote {
    ...
    fn encrypt_to(&self, peer: &NotePubkey, msg: &[u8]) -> Vec<u8> {
        ...
        // AAD = epk: the ephemeral pubkey is authenticated by the tag (cannot be substituted, §3.1.3).
        let ct = cipher
...
mod tests {
    ...
    fn from_seed_is_deterministic_and_distinct() {
        // Directive 7: the same root key → the same note (persistent identity across runs).
        let seed = [7u8; 32];
    ...
    /// #133 §5 regression: the two `LocalNote` x25519 derivations are NON-INTEROPERABLE. `from_seed`
    ...
    fn from_seed_x25519_is_not_the_montgomery_handover_form() {
        ...
        assert_ne!(
            mpk.x, montgomery,
            "from_seed (HKDF) x25519 must differ from Montgomery(ed) — the non-interop invariant (#133 'do not mix')"
        );
    ...
    fn derive_is_deterministic_reproducible_and_distinct_per_index() {
        // Directive 7 §acceptance 2/6: deriv(root, i) is reproducible — the same root+index → the same
    ...
    fn note_tree_enumerates_distinct_reproducible_nodes() {
        // Directive 7: the tree is enumerable and reproducible. From one key — several DIFFERENT notes;
    ...
    fn from_secret_hex_matches_seed_and_rejects_bad() {
        ...
        // A wrong key → an explicit rejection (directive 7 §acceptance 5), not silent generation.
        assert!(matches!(
    ...
    fn loaded_note_is_fully_functional() {
        ...
        // (the HKDF-derived x25519 is compatible with the handover §3.1.3).

//! Seller-driven tick-advance orchestrator (issue #37).
//!
...
//! deal never reaches `Streaming`, so a STOP burns (`BurnBoth`, §3.1.2) instead of settling by-fact —
//! that is the gap reported in #37. `advance()` is **seller-only** on-chain
...
//!
//! The windows are **per-deal** (§9.1, dynamic since contracts 4.0.5): production mirrors them from
...
//! buyer lifecycle are the next increments tracked in `directives/37-tick-advance-orchestrator.report.md`.
...
/// Per-deal advance windows, mirrored from `TokenContract.getConfig()` (§9.1).
...
/// The probe-acceptance window is **fixed** on-chain (§9.1, 4.0.5): `advance()` uses
...
impl AdvanceWindows {
    ...
    /// probe phase always waits the fixed [`PROBE_WINDOW`] (§9.1), the stream phase uses the deal's
    /// `settleWindow`. This is the seller driver's per-deal window construction (issue #37, increment 2:
...
pub async fn drive_advance(
    ...
    // §3.1.2 + #37/#89: finalized ticks must NEVER exceed delivered canonical ticks. The delivery counter is
    ...
    // §3.3/#89: each subsequent tick is finalized after the (dynamic) settle window, but ONLY up to the
    // number of canonical ticks actually delivered — a timer firing does not entitle the seller to a tick
    // the buyer never received (#37). The loop ends on a clean external close (buyer STOP / self-destruct), the
...
mod tests {
    ...
    /// `from_settle_window` pairs the **fixed** §9.1 probe window (180s) with the deal's **dynamic** settle
    /// window (from `getConfig().settleWindow`) — the seller driver's per-deal window construction (#37).
    #[test]
    fn advance_windows_from_settle_window_fixes_probe_keeps_settle() {
        ...
        assert_eq!(
            ...
            "probe window is the fixed §9.1 constant"
        );
    ...
    /// used to prove `drive_advance` propagates it instead of reporting a successful loop (#37 money-path
    /// safety, PR #40 review). Probe acceptance succeeds; the stream reports no snapshot (not a close).
    ...
    /// never finalizes more ticks than were delivered (#37, PR #47 review).
    ...
    /// #37/#89 money-path safety: `drive_advance` finalizes **at most** the delivered tick count, never the
    ...
    /// #37/#89 (PR #48 review): the first token can arrive AFTER the probe window — the driver must WAIT for it
    ...
    /// #198 regression: after `open_stream`, a matched buyer may not have connected yet. Empty delivery is not a
    ...
    /// #37/#89 (PR #48 review): tokens delivered just before `done` must be finalized — the driver re-reads

//! Gateway channel confidentiality (§3.1.3): a self-signed TLS certificate brought up
//! at gateway startup. There is no PKI — trust in the certificate comes from the encrypted handover
//! (§3.1): the **fingerprint** (SHA-256 over DER) is placed next to the endpoint, and the buyer pins it.
//!
...
pub struct GatewayTls {
    ...
    pub key_pem: String,
    /// Certificate fingerprint: SHA-256 over DER, hex. Placed into the handover (§3.1.3).

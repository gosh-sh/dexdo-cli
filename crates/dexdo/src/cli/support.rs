...
/// decide the consumer-API content-identity policy for a frame model, BEFORE opening the API. Pure (no
/// I/O -- `has_ref_key` is computed by the caller from env) so it is unit-testable. A seller can declare the
/// correct model NAME yet serve a cheaper model; only the CONTENT layers(B8 fingerprint / B7-full reference)
/// catch that. Policy:
/// - `--mock-model` -> `Skip`;
/// - an exact model id with a B8 fingerprint OR a B7 reference key in env -> `Probe`(run the one-per-deal content gate);
/// - otherwise `--allow-unverified-model` -> `Skip` with a loud warning(name-only, the operator opted in);
/// - else **refuse**(fail closed): buying on name-only evidence is rejected.
pub(crate) fn content_check_policy(
    frame_model: &str,
    identity_model: Option<&str>,
    mock_model: bool,
    allow_unverified: bool,
    has_ref_key: bool,
) -> anyhow::Result<dexdo::buyer::api::ContentCheck> {
    use dexdo::buyer::api::ContentCheck;
    let family = dexdo::buyer::verify::family_of(frame_model);
    let identity_model = identity_model.unwrap_or(frame_model);
    if mock_model {
        return Ok(ContentCheck::Skip);
    }
    let has_b8 = dexdo::buyer::verify::default_probe(identity_model).is_some();
...
mod content_policy_tests {
    use super::content_check_policy;
    use dexdo::buyer::api::ContentCheck;

    // finding 4: fail-loud content-identity policy(pure). A seller can declare the correct model NAME
    // yet serve a cheaper model; only the CONTENT layers(B8 fingerprint / B7 reference) catch that. A real
    // model identity with neither must FAIL CLOSED unless the operator opts into name-only identity.

    #[test]
    fn mock_model_skips() {
        // --mock-model: tokens are fake by design -> no content check, regardless of the frame model.
        assert_eq!(
            content_check_policy("qwen/qwen3-32b", None, true, false, false).unwrap(),
            ContentCheck::Skip
        );
    }

    #[test]
    fn qwen_b8_fingerprint_probes() {
        // The explicit qwen3-32b frame model has a B8 fingerprint(default_probe is Some) -> Probe even with no
        // reference key.
        assert_eq!(
            content_check_policy("qwen--qwen3--32b", None, false, false, false).unwrap(),
            ContentCheck::Probe {
    ...
    #[test]
    fn registry_backed_qwen_identity_probes_by_registry_model() {
        assert_eq!(
    ...
    fn unknown_qwen_variant_does_not_inherit_qwen3_fingerprint() {
        assert_eq!(
            content_check_policy("qwen--qwen3.6--27b", None, false, true, false).unwrap(),
            ContentCheck::Skip
        );
    }

    #[test]
    fn real_family_no_fingerprint_no_key_fails_closed() {
        // llama: a real family with NO B8 fingerprint and NO B7 reference; has_ref_key=false and the operator
        // did NOT pass --allow-unverified-model -> refuse to buy on name-only evidence(fail closed).
        let r = content_check_policy("meta-llama/llama-3.1-8b", None, false, false, false);
        assert!(r.is_err(), "name-only model must fail closed, got {r:?}");
    }

    #[test]
    fn allow_unverified_downgrades_to_skip() {
        // Same name-only family, but the operator explicitly accepted name-only identity -> Skip(loud warn).
        assert_eq!(
            content_check_policy("meta-llama/llama-3.1-8b", None, false, true, false).unwrap(),
            ContentCheck::Skip
        );
    }

    #[test]
    fn reference_key_enables_probe() {
        // A B7 reference key in env(has_ref_key=true) enables the content gate even without a B8 fingerprint.
        assert_eq!(
            content_check_policy("meta-llama/llama-3.1-8b", None, false, false, true).unwrap(),
            ContentCheck::Probe {

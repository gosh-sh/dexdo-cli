...
/// @notice Async pull sink: the weekly median is handed back here.
...
/// @notice Both sides rest (SELL offer = one-deal slot; BUY = limit/subscription
...
/// SELL side's token_contract with the BUY side's note bound -> the deal
/// streams off-book. The book also keeps a daily-VWAP / weekly-median
/// reference price.
...
contract InferenceOrderBook is AiRegistryModifiers {
    string constant version = "4.0.18";
    ...
    uint256 constant NOTE_CODE_HASH  = 0x1d2fcae0a1a7bc8af4e39992fbf0eda7bc2e7ff3397e44500ff03b57247d732f;
    ...
    uint256 constant TOKEN_CONTRACT_CODE_HASH  = 0x1fa8c398d5379a5bb31380c3c2b5b8dd36a7c0a1e7af94d89cce5a2e9458eb4b;
    ...
    uint256 constant ROOT_MODEL_CODE_HASH  = 0x1425d0ce8e82fc19ff23a721b2ef11befdb493c0901a9f63f4f903abca3d5de7;
    ...
    struct Order {
        ...
        uint256 buyerPubkey; // BUY: buyer note pubkey; SELL: 0
    ...
    // One resting SELL per deal TokenContract. A TC is a single one-shot deal (funded once,
    // `_funded` latch), so two live SELL offers bound to the same TC are wrong -- only the first
    // fill becomes a real deal; the rest phantom-fill(their fund bounces back). This set marks a
    // TC address as having a live resting SELL. Value is `bool` (not the orderId -- it is never read
    // back). Set when a SELL rests(`_insertIntoBook`), cleared on ANY removal (`_removeFromBook` --
    // fill/cancel/expire all route through it), so cancel-then-repost of the same TC works and the
    // set stays O(active SELLs).
    mapping(address => bool) _sellTcInUse;
    ...
    struct QueueEntry {
        ...
        uint256 buyerPubkey; // BUY: buyer note pubkey; else 0
        bool    isBuy;
        uint8   flags;
    ...
    // -- subscription state(keyed by resting BUY order id) --
    struct Sub {
        uint64  periodStart;
        uint8   curCycle;
        uint128 cycleBudget;
    ...
    function _insertIntoBook(uint128 orderId, Order o) private {
        ...
        _orders[orderId] = o;
        // Mark this SELL's deal contract as in-use(BUY has tokenContract == 0).
        if (!o.isBuy) { _sellTcInUse[o.tokenContract] = true; }
    ...
    function _removeFromBook(uint128 orderId) private {
        ...
        // Free this SELL's deal contract so a fresh offer can bind it again(cancel/expire/fill).
        if (!o.isBuy) { delete _sellTcInUse[o.tokenContract]; }
        // Drop any subscription metadata for this order on EVERY removal path -- fill, expire, AND
    ...
    function _match(
        ...
        while (it.hasValue() && remaining > 0) {
            ...
            while (cur != 0 && remaining > 0) {
                ...
                // subscription maker(resting buy): roll cycles + cap by cycle budget.
                ...
                if (takerIsBuy) {
                    ...
                    // = mk.escrow - cost, set above) must NOT strand.
                    if (makerSub) {
                        // an early full-fill forfeits ONLY the CURRENT cycle's own
    ...
    function _doPlaceHead() private returns (bool) {
        ...
        if (firstRun) {
            // One resting SELL per deal TokenContract: reject a second offer bound to a TC that
            // already has a live resting SELL -- it could only phantom-fill(its match refunds).
            // A SELL carries no ECC escrow, so return the seller's attached
            // placement value and skip: no order is created, no placed-event/mirror is emitted.
            if (!e.isBuy && _sellTcInUse.exists(e.tokenContract)) {
                e.owner.transfer({value: 1 vmshell, flag: 1, bounce: false});
                return false;
            }
    ...
    // ========================================================
    // subscription(semantic order)
    // ========================================================

    /// @notice subscription: a resting limit buy whose budget is throttled into
    ...
    /// placement. Renewal = client re-places; `autoRenew` is a hint.
